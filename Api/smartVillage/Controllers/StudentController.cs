...
namespace smartVillage.Controllers
...

    {
        ...
        public class StudentController : ApiController
        {
            [HttpPost]
            [Route("SaveStudent")]
            public string SaveStudent(StudentDto dataDto)
            {
                ...
                using (smartVillageDB context = new smartVillageDB())
                {
                    ...
                    if (dataDto.Id > 0)
                    {
                        var editData = context.Students.FirstOrDefault(x => x.Id == dataDto.Id);
                    ...
                    else
                    {
                        ...
                        var old = context.Students.FirstOrDefault(x => x.IsActive && x.Email == dataDto.Email);
                        if (old != null)
                        {
            ...
            public List<StudentDto> GetStudents()
            {
                ...
                using (smartVillageDB context = new smartVillageDB())
                {
                    ...
                    var dataList = context.Students.Where(x => x.IsActive == true).Select(x => new StudentDto
                    {
                        Id = x.Id,
                        Name = x.Name,
                        MobileNo = x.MobileNo,
            ...
            public bool DeleteStudent(long id)
            {
                using (smartVillageDB context = new smartVillageDB())
                {
                    var deleteData = context.Students.FirstOrDefault(x => x.Id == id);
            ...
            public StudentDto GetStudentById(long id)
            {
                using (smartVillageDB context = new smartVillageDB())
                {
                    var dataList = context.Students.Where(x => x.IsActive == true && x.Id == id)
